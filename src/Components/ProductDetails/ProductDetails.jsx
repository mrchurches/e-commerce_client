...
export default function ProductDetails() {
  ...
  useEffect(()=>{
    const order = window.screen.width > 760 ? "order-0" : "order-2"
    setOrder(order)
  })
      ...
      {game.name && (
        <div class="row">
          <div class={"col-md-5 " + order}>
            <div class="project-info-box col mt-0">
              <h4>{game.name}</h4>
              <p className='description'>{game.description}</p>
            </div>

            <div class="project-info-box col ">
              <div className='divp1'>
          ...
          <div class="col-md-7 order-1">
            ...
            <div class="col order-1 project-info-box">
              <p className='p3'><b>Platforms:</b>{game.platforms?.map((e, i) => (<span key={i}> {e.name}</span>))} </p>
            ...
            <div className='verticalScrollable1'>
              {reviews && reviews.map((e, i) => {
                return(<ReviewCard key={i} username={e.username} rating={e.rating} description={e.description} userImg={e.profile_pic}/>)
              })}
            </div>
